...
class QKNormCrossAttention(nn.Module):
    ...
    pretrained with nn.MultiheadAttention can be grafted in via
    `graft_from_mha_state` followed by `set_magnitude_scales`. Both are library
    helpers for a caller to use: no script in this repository calls them, and
    `train_rfbg.py --resume <non-qk-norm.pt> --qk-norm` will fail the strict
    load_state_dict on the missing scale parameters rather than graft for you.

    Logits become cos(q,k) * q_scale * k_scale / sqrt(head_dim), so they no longer
    depend on the magnitude of the projection weights. They are NOT bounded in
    absolute terms: q_scale and k_scale are learnable, and nothing constrains their
    product. In the released checkpoint block 0 learnt |q_scale| up to 52.1 and
    |k_scale| up to 409.8, a logit bound near 3.8e3, while blocks 1-3 stayed at
    4-6; block 0's attention is correspondingly concentrated. The mechanism removes
    one route to logit blow-up, not every route. The learnable per-head scales are
    init to the pretrained ||q||,||k|| magnitudes at graft time so the graft is
    near-identity (short heal); default init 1.0 for from-scratch.
