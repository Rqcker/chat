...
This repository is an **open reimplementation** of the paper, not the training
run behind its tables. Where the code differs from the paper, it is listed in
[`DOCS.md`](DOCS.md); measured reproduction numbers are in the
[v1.0.0 release notes](https://github.com/Rqcker/chat/releases/tag/v1.0.0).
...
|---|---|---|
| `checkpoints/rfbg_qknorm_ft_030500.pt` | 13 MB | RFBG: HDTF cold start, then a dyadic fine-tune on NoXI |
| `checkpoints/sfbg_pre_020000.pt` | 5.3 MB | SFBG: 20k-step HDTF cold start |

Both are reproduction artefacts trained at reduced scale, not the paper's models.
Each stores its own config, so the architecture is read from the file. See
...
released for non-commercial research use only, and are derived from HDTF and
NoXI, whose own terms bind any use of them. Third-party components are installed
