...
This repository is an open reimplementation of CHAT (ECCV 2026), not the training
run behind the paper's tables. The default configuration is the paper's method,
minus the gaps listed below. Everything beyond the paper is off unless you switch
it on.

Measured reproduction numbers, and the protocol each one depends on, are in the
