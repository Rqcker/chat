...
def load_sfbg(ckpt_path: str, device: str, weights: str = "auto"):
    ...
    if weights == "raw":
        ...
        if "model_ema" not in state:  # never silently fall back to raw
            raise SystemExit(f"{ckpt_path} carries no EMA copy; use --weights raw or auto")
        sd = state["model_ema"]
    else:  # auto: prefer EMA when present, else raw
        sd = state.get("model_ema", state["model"])
    model.load_state_dict(sd)
    return model
