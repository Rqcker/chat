...
class LatentWindows(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # draw a pseudo-partner uniformly from every other clip; randrange(n-1)
        # here would never return the last clip
        if len(self.files) > 1:
            j = self.rng.randrange(len(self.files) - 1)
            if j >= idx:
                j += 1
        else:
            j = idx
...
def main() -> None:
    ...
    ap.add_argument("--embed-dim", type=int, default=None, help="capacity override (default 128 = the released 1.7M model)")
    ...
    # capacity overrides; the defaults reproduce the released 1.7M model
