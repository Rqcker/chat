...
def main():
    ...
    ap.add_argument("--ref-per-video", type=int, default=None,
                    help="cap on clips taken from each reference video. Default: 3 with "
                         "--ref-reframe, otherwise uncapped. The two sides use different "
                         "caps by default, which is a sampling asymmetry -- set this "
                         "explicitly to make both sides match.")
    ...
    ref_per_video = args.ref_per_video if args.ref_per_video is not None else (3 if args.ref_reframe else 0)
    gen_per_video = 0
    if ref_per_video != gen_per_video:
        print(f"NOTE: reference takes <={ref_per_video} clips per video, generated is "
              f"uncapped per video. The two sides are sampled differently; pass "
              f"--ref-per-video 0 to match them.")
    ...
    print("NOTE: a real-video floor control landed inside the generated band here, so")
    print("      this number is protocol-passing, not evidence of generation quality.")
    if gen.shape[0] < 64:
