...
def main():
    ...
    ap.add_argument("--paper-lsed", action="store_true",
                    help="additionally report the supplementary's LSE-D definition, the mean "
                         "L2 distance between consecutive SyncNet lip embeddings, in raw, "
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # best offset, which is what this script computes as the headline LSE-D. A draft
        # supplementary that main.tex never inputs, and which is therefore not part of the
        # compiled paper, instead described LSE-D as "the average L2 distance between
        # consecutive lip feature vectors" — a temporal-consistency measure, a different
        # quantity. This flag captures SyncNet's visual embeddings and reports every
        # reasonable reading of that wording so the question can be settled by scale rather
